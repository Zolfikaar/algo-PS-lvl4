...
int DifferenceBetweenTwoDates(sDate Date1, sDate Date2, bool IncludeEndDay = false)
{
    ...
    if (CheckWhichDateIsLess(Date1, Date2)) {
        ...
        while (CheckWhichDateIsLess(Date2, Date1)) {
            Days--;
...
int main()
{
    ...
    int DiffDate = DifferenceBetweenTwoDates(Date1, Date2);
    int DiffDateWithEndDay = DifferenceBetweenTwoDates(Date1, Date2, true);


    cout << "\nDifference is: " << DiffDate << " Day(s)." << endl;
    cout << "Difference (including end day) is: " << DiffDateWithEndDay << " Day(s).";
}
